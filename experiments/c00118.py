...
class PPM_Record():
    ...
    def setFilename(self, basename=None, useTime=True):
        dirname='/reg/neh/operator/xcsopr/experiments/xcsx43118/powermeterData/'
        if basename is None:
...
class User():
    ...
    def relgridscan(self, motor1, start1, stop1, nstep1, motor2, start2, nstop2, nstep2, nEvents, record=True):
        daq.configure(nEvents, record=record, controls=[motor1, motor2])
        RE(rel_grid_scan([daq], motor1, start1, stop1, nstep1, motor2, start2, nstop2, nstep2, snake_axes=True))
